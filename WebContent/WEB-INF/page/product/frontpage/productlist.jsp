...
<c:forEach items="${pageView.records}" var="entry">	
        <div class="detail">
           <div class="goods" style="cursor:hand;background:url(<c:forEach items="${entry.styles}" var="img"><%=request.getContextPath() %>/${img.imageFullPath}</c:forEach>) center center no-repeat"><a href='<s:url action="productview" namespace="/center/product" includeParams="none"/>?productid=${entry.id}' target="_blank">
           ...
           <div class="save_number"><s>€${entry.marketprice}</s>　<strong><em>€${entry.sellprice}</em></strong>　Save: ${entry.savedPrice}</div>
           <div class="an_img" align="center"><a href='<s:url action="productview" namespace="/center/product" includeParams="none"/>?productid=${entry.id}'><img src="<%=request.getContextPath() %>/images/sale.gif" width="84" height="24" border="0" class="a_1" /></a></div>
        </div>
</c:forEach>
<!----------------------LOOP END------------------------------->
		<div class='emptybox'></div>
